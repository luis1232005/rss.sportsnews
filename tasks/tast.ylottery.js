...
// data 先从：http://live.aicai.com/static/no_cache/jc/zc/data/gudinghh_sp.html这里面取值
    ...
    try {
        ...
        if (ctObj.status && ctObj.status == "success" && ctObj.result && ctObj.result.jsbf_matchs) {
            ...
            $(".tbody_body tr").each(function () {
                ...
                var week = matchNo.substring(0,2);
                var weekIndex = 0;
                if(weekIndex == 0){
                    weekIndex = util.getWeekIndexByName(week);
                }
                var matchSeq = matchNo.substring(2);
                var spIndex = weekIndex + "-" + matchSeq;
...
function fetchLottery(dateObj) {
    ...
    F.createPromiseList([{url: peiLvUrl}, {url: gamesUrl},{url:preSpUrl}])
        .then(function (cts) {
            var items = null;
            if (cts[0] && cts[1]) {
